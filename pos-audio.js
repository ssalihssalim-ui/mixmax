...
function posToggleVoiceSearch() {
    ...
    if (!navigator.onLine) { alert('⚠️ La recherche vocale nécessite une connexion internet.'); return; }
    var micBtn = document.getElementById('posMicBtn'); if (isRecording) { posStopVoiceSearch(); return; }
    requestMicrophonePermission().then(hasPermission => { if (!hasPermission) { alert('❌ Accès au microphone refusé.'); return; } posStartVoiceRecording(); });
}
