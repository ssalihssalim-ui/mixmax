// ==================== POS-AUDIO.JS v8.1.2 FINAL - RECHERCHE PRODUIT INSTANTANÉE ====================
// Mixmax Minimarket - Barre traitement disparaît + micro redémarre auto + 200ms inter-phase
// Optimisé pour recherche produit ultra‑rapide (nom + description)
...
// ==================== PARSER VOCAL v8.1 (RECHERCHE PRODUIT INSTANTANÉE) ====================
function parseVoiceCommand(transcript) {
    ...
    // 1. Commandes de navigation (prioritaires pour changer de page)
    ...
    // 2. Recherche produit ultra‑rapide en étape 1 (mode recherche)
    if (window.posStep === 1 && voiceMode === 'search') {
        if (window.posProductsList && window.posProductsList.length) {
            ...
            for (var i = 0; i < window.posProductsList.length; i++) {
                ...
                if (nom.includes(transcript) && nom.length > bestLen) {
                    bestMatch = p; bestLen = nom.length;
                }
                if (desc.includes(transcript) && desc.length > bestLen) {
                    bestMatch = p; bestLen = desc.length;
                }
            }
            if (bestMatch) {
        ...
        // pas de produit -> on continue pour éventuellement un client, mais on évite de déclencher autre chose
    }

    // 3. Détection de nombres / modes de paiement (seulement si nécessaire)
    ...
    // 4. Commandes spécifiques aux pages Ventes / Crédits
    ...
    // 5. Commandes d'action générales
    ...
    // 6. Recherche produit / client (étape 2 ou si pas trouvé plus haut)
    if (window.posStep === 2) {
        if (window.posAllClients) {
            var fc3 = fastFindClient(transcript);
            if (fc3.length > 0) { var b3 = fc3[0]; for (var j = 0; j < fc3.length; j++) { if ((fc3[j].nom + ' ' + fc3[j].prenom).toLowerCase().indexOf(transcript) !== -1) { b3 = fc3[j]; break; } } return { type: 'client', client: b3 }; }
        }
        if (window.posProductsList) {
            var fp2 = null, bml2 = 0;
            for (var i2 = 0; i2 < window.posProductsList.length; i2++) {
                var p2 = window.posProductsList[i2];
                var nom2 = (p2.nom || '').toLowerCase();
                var desc2 = (p2.description || '').toLowerCase();
                if ((nom2.includes(transcript) || desc2.includes(transcript)) && nom2.length > bml2) { fp2 = p2; bml2 = nom2.length; }
            }
            if (fp2) return { type: 'product', product: fp2 };
        }
    } else {
        if (window.posProductsList) {
            var fp = null, bml = 0;
            for (var i = 0; i < window.posProductsList.length; i++) {
                ...
                var nom = (p.nom || '').toLowerCase();
                var desc = (p.description || '').toLowerCase();
                if ((nom.includes(transcript) || desc.includes(transcript)) && nom.length > bml) { fp = p; bml = nom.length; }
            }
            if (fp) return { type: 'product', product: fp };
        }
        if (window.posAllClients) {
            var fc4 = fastFindClient(transcript);
            if (fc4.length > 0) { var b4 = fc4[0]; for (var j = 0; j < fc4.length; j++) { if ((fc4[j].nom + ' ' + fc4[j].prenom).toLowerCase().indexOf(transcript) !== -1) { b4 = fc4[j]; break; } } return { type: 'client', client: b4 }; }
        }
    }
...
// ==================== GESTIONNAIRE DE COMMANDES (barre disparaît + micro redémarre) ====================
...
// ==================== TOGGLE MICRO (barre disparaît + micro redémarre auto) ====================
...
console.log('🎤 Mixmax Minimarket - Module vocal v8.1.2 (recherche produit instantanée)');
