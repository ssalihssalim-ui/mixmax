// ==================== POS-AUDIO.JS v9.0 DARIJA - RECONNAISSANCE VOCALE DARIJA MAROCAINE ====================
// Mixmax Minimarket - Interface vocale 100% darija (arabe dialectal marocain)
...
var voiceModeMessage = '🎤 البحث الصوتي نشيط'; // Recherche vocale active
...
var paymentKeywords = {
    'espece': ['كاش', 'فلوس', 'نقد', 'ليكاش', 'كيش', 'cash', 'liquide'],
    'credit': ['كريدي', 'دين', 'سلف', 'بكريدي', 'credit'],
    'partiel': ['جزء', 'شوية', 'تقسيط', 'مويتي', 'بارت', 'partiel']
};

// Nombres en darija (transcription phonétique + écriture arabe)
var numberMap = {
    'واحد': 1, 'جوج': 2, 'جوج': 2, 'تلاتة': 3, 'ربعة': 4, 'خمسة': 5,
    'ستة': 6, 'سبعة': 7, 'تمنية': 8, 'تسعود': 9, 'عشرة': 10,
    'حداش': 11, 'طناش': 12, 'تلتاش': 13, 'ربعطاش': 14, 'خمسطاش': 15,
    'ستطاش': 16, 'سبعطاش': 17, 'تمنطاش': 18, 'تسعطاش': 19,
    'عشرين': 20, 'تلاتين': 30, 'ربعين': 40, 'خمسين': 50,
    'ستين': 60, 'سبعين': 70, 'تمنين': 80, 'تسعين': 90, 'مية': 100,
    // variantes phonétiques occidentales
    'wahed': 1, 'juj': 2, 'tlata': 3, 'rbaa': 4, 'khemsa': 5,
    'setta': 6, 'sebaa': 7, 'tmnya': 8, 'tsaud': 9, 'ashra': 10
};
...
function buildClientIndex() {
    ...
    window.posAllClients.forEach(function(c) {
        if (!c || !c.id) return;
        // Prend en compte le nom, prénom, téléphone, description, et un éventuel champ darija
        var allText = (c.nom + ' ' + c.prenom + ' ' + c.telephone + ' ' + (c.description || '') + ' ' + (c.nom_darija || '') + ' ' + (c.prenom_darija || '')).toLowerCase();
        ...
        // Ajouter la version darija du nom complet
        if (c.nom_darija || c.prenom_darija) {
            var fullNameDarija = ((c.nom_darija || '') + ' ' + (c.prenom_darija || '')).toLowerCase().trim();
            if (fullNameDarija.length >= 2) {
                if (!clientSearchIndex[fullNameDarija]) clientSearchIndex[fullNameDarija] = [];
                if (clientSearchIndex[fullNameDarija].indexOf(c) === -1) clientSearchIndex[fullNameDarija].push(c);
            }
        }
    });
...
function fastFindClient(query) {
    ...
    if (results.length === 0 && window.posAllClients) {
        results = window.posAllClients.filter(function(c) {
            var searchText = (c.nom || '') + ' ' + (c.prenom || '') + ' ' + (c.description || '') + ' ' + (c.telephone || '') + ' ' + (c.nom_darija || '') + ' ' + (c.prenom_darija || '');
            return searchText.toLowerCase().indexOf(q) !== -1;
        });
    }
    return results;
}
...
function showVoiceFlowIndicator(phase) {
    ...
    setTimeout(function() {
        var colors = {
            'product': { bg: '#dcfce7', border: '#16a34a', text: '🎤 ڭول المنتوج' },
            'quantity': { bg: '#fef3c7', border: '#f59e0b', text: '🔢 ڭول الكمية' },
            'client': { bg: '#e0e7ff', border: '#4f46e5', text: '👤 ڭول الزبون' },
            'payment_mode': { bg: '#e0e7ff', border: '#4f46e5', text: '💳 كاش، كريدي ولا بارت؟' },
            'payment_amount': { bg: '#e0e7ff', border: '#4f46e5', text: '💰 ڭول المبلغ' },
            'confirm': { bg: '#f3e8ff', border: '#9333ea', text: '✅ ڭول "صيفط" باش تأكد' },
            'idle': { bg: '#f0fdf4', border: '#16a34a', text: '🎤 سمع...' }
        };
        var c = colors[phase] || colors['idle'];
        voiceFlowIndicator = document.createElement('div');
        voiceFlowIndicator.id = 'voiceFlowIndicator';
        voiceFlowIndicator.style.cssText = 'position:fixed;top:12px;left:50%;transform:translateX(-50%);background:' + c.bg + ';border:3px solid ' + c.border + ';border-radius:30px;padding:12px 28px;font-size:1rem;font-weight:700;z-index:9998;box-shadow:0 4px 25px rgba(0,0,0,0.2);text-align:center;color:#1e293b;opacity:0;transition:opacity 0.15s;white-space:nowrap;direction:rtl;';
...
// Barre rouge "Traitement..." en darija
function showProcessingIndicator() {
    ...
    ind.style.cssText = 'position:fixed;top:12px;left:50%;transform:translateX(-50%);background:#fef2f2;border:3px solid #ef4444;border-radius:30px;padding:12px 28px;font-size:1rem;font-weight:700;z-index:9998;box-shadow:0 4px 25px rgba(0,0,0,0.2);text-align:center;color:#991b1b;opacity:0;transition:opacity 0.1s;white-space:nowrap;direction:rtl;';
    ind.textContent = '⏳ جاري المعالجة... ما تهدرش'; // Traitement... Ne parlez pas
...
// ==================== GESTION CRÉDITS (COMPACTE) - messages darija ====================
function activateCreditSelection(){ creditDeletePending=null; window.creditSelectionMode=true; window.creditSelectedIndex=-1; window.creditPaymentStep='idle'; if(typeof window.renderCreditsTable==='function') window.renderCreditsTable(); showVoiceResult('📋 وضع الاختيار'); }
function selectCreditLine(n){ creditDeletePending=null; var d=window.filteredCredits||window.allCreditsData||[]; var i=n-1; if(i<0||i>=d.length){ showVoiceResult('❌ سطر '+n); return; } if(!window.creditSelectionMode){ showVoiceResult('⚠️ اختار أولاً'); return; } window.creditSelectedIndex=i; window.creditPaymentStep='selection'; window.creditPaymentAmount=0; if(typeof window.renderCreditsTable==='function') window.renderCreditsTable(); var c=d[i]; showVoiceResult('✅ سطر '+n+' - '+(c.clientName||'')+' - '+(c.remainingAmount||c.total||0).toFixed(2)+' درهم'); }
function markCreditForPayment(){ creditDeletePending=null; if(window.creditSelectedIndex<0){ showVoiceResult('⚠️ ما كاين حتى سطر'); return; } var d=window.filteredCredits||window.allCreditsData||[]; var c=d[window.creditSelectedIndex]; if(!c){ showVoiceResult('❌ ما لقيتوش'); return; } if(c.paid){ showVoiceResult('⚠️ مخلص أصلاً'); return; } window.creditPaymentStep='payment'; var r=c.remainingAmount||c.total||0; showVoiceResult('💳 باقي: '+r.toFixed(2)+' درهم'); var z=document.getElementById('creditPaymentZone'); if(z){ z.style.display='block'; var info=document.getElementById('creditPaymentInfo'); if(info) info.textContent='الزبون: '+(c.clientName||'مجهول')+' | الباقي: '+r.toFixed(2)+' درهم'; var inp=document.getElementById('creditPaymentAmountInput'); if(inp){ inp.value=''; inp.focus(); inp.select(); } } }
function setCreditPaymentAmount(a){ if(window.creditSelectedIndex<0){ showVoiceResult('⚠0}; return; } if(a<=0){ showVoiceResult('❌ مبلغ غير صالح'); return; } window.creditPaymentAmount=a; window.creditPaymentStep='amount'; var inp=document.getElementById('creditPaymentAmountInput'); if(inp) inp.value=a; showVoiceResult('💰 '+a.toFixed(2)+' درهم'); }
function validateCreditPayment(){ if(window.creditSelectedIndex<0){ showVoiceResult('⚠️ ما كاين حتى كريدي'); return; } var inp=document.getElementById('creditPaymentAmountInput'); var a=parseFloat(inp?inp.value:window.creditPaymentAmount); if(isNaN(a)||a<=0){ showVoiceResult('❌ مبلغ غير صالح'); return; } var d=window.filteredCredits||window.allCreditsData||[]; var c=d[window.creditSelectedIndex]; if(!c){ showVoiceResult('❌ ما لقيتوش'); return; } var r=c.remainingAmount||c.total||0; if(a>r){ showVoiceResult('⚠️ كتر من الباقي'); return; } var nr=r-a; var p=nr<=0.01; CacheDB.write('credits',c.id,{paid:p,remainingAmount:Math.max(0,nr),amountGiven:(c.amountGiven||0)+a,paidAt:firebase.firestore.FieldValue.serverTimestamp(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()},'update').then(function(){ showVoiceResult(p?'✅ تم الأداء!':'✅ خلص. باقي: '+nr.toFixed(2)+' درهم'); window.creditPaymentStep='idle'; window.creditSelectedIndex=-1; window.creditPaymentAmount=0; window.creditSelectionMode=false; var z=document.getElementById('creditPaymentZone'); if(z) z.style.display='none'; if(typeof window.loadCredits==='function') window.loadCredits(); CacheDB.sync(); }).catch(function(e){ showVoiceResult('❌ خطأ'); }); }
function closeCreditSelection(){ creditDeletePending=null; window.creditSelectionMode=false; window.creditSelectedIndex=-1; window.creditPaymentAmount=0; window.creditPaymentStep='idle'; var z=document.getElementById('creditPaymentZone'); if(z) z.style.display='none'; if(typeof window.applyCreditsFilters==='function'){ window.creditsSearch=''; window.currentPages.credits=1; window.filteredCredits=null; window.applyCreditsFilters(); } showVoiceResult('📋 القائمة الكاملة'); }

// ==================== SUPPRESSION AVEC CONFIRMATION darija ====================
function deleteCreditByVoice(lineNumber) { var data=window.filteredCredits||window.allCreditsData||[]; var i=lineNumber-1; if(i<0||i>=data.length){showVoiceResult('❌ سطر '+lineNumber);return;} var c=data[i]; if(!c){showVoiceResult('❌ كريدي ما كاينش');return;} creditDeletePending={lineNumber:lineNumber,credit:c}; showVoiceResult('🗑️ '+(c.clientName||'مجهول')+' ? ڭول صيفط/لا'); }
function deleteSelectedCredit(){ if(window.creditSelectedIndex<0){showVoiceResult('⚠️ ما كاين حتى سطر');return;} deleteCreditByVoice(window.creditSelectedIndex+1); }
async function confirmDeleteCredit(){ if(!creditDeletePending){showVoiceResult('⚠️ ما كاين والو للحذف');return;} var c=creditDeletePending.credit; try{ if(typeof window.deleteCredit==='function'){await window.deleteCredit(c.id);}else{await db.collection('credits').doc(c.id).delete();window.allCreditsData=(window.allCreditsData||[]).filter(function(x){return x.id!==c.id;});if(typeof window.loadCredits==='function') window.loadCredits();} window.creditSelectedIndex=-1;window.creditSelectionMode=false;showVoiceResult('✅ تحذف'); }catch(e){showVoiceResult('❌ خطأ');} creditDeletePending=null; }
function cancelDeleteCredit(){ if(creditDeletePending){creditDeletePending=null;showVoiceResult('↩️ تــراجع');} }

// ==================== DÉTECTION PAIEMENT darija ====================
...
function parseVoiceCommand(transcript) {
    ...
    // Si on attend une quantité ou un mode de paiement
    ...
    // Commandes spécifiques à la page "Ventes" en darija
    if (currentPage === 'Ventes') {
        if (transcript.includes('اليوم') || transcript.includes('نهار')) { window.ventesPeriod = 'today'; window.currentPages.ventes = 1; if (typeof window.applyVentesFilters === 'function') window.applyVentesFilters(); showVoiceResult('📅 اليوم'); return { type: 'ignore' }; }
        if (transcript.includes('هد الاسبوع') || transcript.includes('7 أيام') || transcript.includes('سيمانة')) { window.ventesPeriod = '7'; window.currentPages.ventes = 1; if (typeof window.applyVentesFilters === 'function') window.applyVentesFilters(); showVoiceResult('📅 7 أيام'); return { type: 'ignore' }; }
        if (transcript.includes('هد الشهر') || transcript.includes('30 يوم') || transcript.includes('شهر')) { window.ventesPeriod = '30'; window.currentPages.ventes = 1; if (typeof window.applyVentesFilters === 'function') window.applyVentesFilters(); showVoiceResult('📅 30 يوم'); return { type: 'ignore' }; }
        if (transcript.includes('كلشي') || transcript.includes('تاريخ') || transcript.includes('جميع')) { window.ventesPeriod = 'all'; window.currentPages.ventes = 1; if (typeof window.applyVentesFilters === 'function') window.applyVentesFilters(); showVoiceResult('📅 الكل'); return { type: 'ignore' }; }
        if (transcript.includes('اختيار') || transcript.includes('حدد')) { window.venteSelectionMode = true; window.venteSelectedIndex = -1; if (typeof window.renderVentesTable === 'function') window.renderVentesTable(); showVoiceResult('📋 وضع الاختيار'); return { type: 'ignore' }; }
        if (window.venteSelectionMode) {
            var lm = transcript.match(/(?:سطر|رقم)\s+([\u0600-\u06FF\u0030-\u0039]+)/i); if (lm) { var ns = lm[1], num = parseInt(ns); if (isNaN(num)) { for (var w in numberMap) { if (ns.toLowerCase() === w) { num = numberMap[w]; break; } } } if (!isNaN(num) && num > 0) { window.venteSelectedIndex = num - 1; if (typeof window.renderVentesTable === 'function') window.renderVentesTable(); showVoiceResult('✅ سطر ' + num); return { type: 'ignore' }; } }
            var an = transcript.match(/\b(\d+)\b/), num = null; if (an) { num = parseInt(an[1]); } else { for (var w in numberMap) { if (transcript.includes(w)) { num = numberMap[w]; break; } } } if (num !== null && !isNaN(num) && num > 0) { window.venteSelectedIndex = num - 1; if (typeof window.renderVentesTable === 'function') window.renderVentesTable(); showVoiceResult('✅ سطر ' + num); return { type: 'ignore' }; }
        }
        if (transcript.includes('تفاصيل') || transcript.includes('ديتاي')) { if (window.venteSelectedIndex >= 0) { var data = window.filteredVentes || window.allVentesData || []; var vente = data[window.venteSelectedIndex]; if (vente) { if (typeof window.editVente === 'function') { window.editVente(vente.id); } else { showVoiceResult('📋 تفاصيل'); } return { type: 'ignore' }; } } showVoiceResult('⚠️ اختار سطر أولاً'); return { type: 'ignore' }; }
        if (transcript.includes('غلق') || transcript.includes('رجع')) { if (window.venteSelectionMode) { window.venteSelectionMode = false; window.venteSelectedIndex = -1; if (typeof window.renderVentesTable === 'function') window.renderVentesTable(); showVoiceResult('📋 تقفل'); return { type: 'ignore' }; } }
        if (transcript.includes('پوانت') || transcript.includes('كافي') || transcript.includes('كيسا') || transcript.includes('pos') || transcript.includes('رجع للكيسا')) { return { type: 'navigate', page: 'pos' }; }
        if (transcript.includes('كريدي') || transcript.includes('ديون') || transcript.includes('قائمة الكريدي')) { return { type: 'navigate', page: 'credits' }; }
        if (transcript.includes('داشبورد') || transcript.includes('رئيسية') || transcript.includes('home')) { return { type: 'navigate', page: 'dashboard' }; }
        if (transcript.includes('كوموندات') || transcript.includes('طلبات')) { return { type: 'navigate', page: 'commandes' }; }
        if (transcript.includes('منتوجات') || transcript.includes('سلع') || transcript.includes('produits')) { return { type: 'navigate', page: 'products' }; }
        if (transcript.includes('زبائن') || transcript.includes('كليان') || transcript.includes('clients')) { return { type: 'navigate', page: 'clients' }; }
        if (transcript.includes('مبيعات') || transcript.includes('بيعات')) { showVoiceResult('✅ أصلاً فالمبيعات'); return { type: 'ignore' }; }
    ...
    // Commandes spécifiques à la page "Crédits" en darija
    if (currentPage === 'Crédits') {
        if (creditDeletePending) { if (transcript.includes('صيفط') || transcript.includes('إيه') || transcript.includes('نعم') || transcript.includes('ok')) { return { type: 'confirm_delete_credit' }; } if (transcript.includes('لا') || transcript.includes('تراجع')) { return { type: 'cancel_delete_credit' }; } showVoiceResult('⚠️ صيفط ولا لا؟'); return { type: 'ignore' }; }
        if (window.creditPaymentStep !== 'payment' && window.creditPaymentStep !== 'amount') {
            if (transcript.includes('پوانت') || transcript.includes('كافي') || transcript.includes('كيسا') || transcript.includes('pos')) return { type: 'navigate', page: 'pos' };
            if (transcript.includes('مبيعات') || transcript.includes('بيعات')) return { type: 'navigate', page: 'ventes' };
            if (transcript.includes('داشبورد') || transcript.includes('رئيسية')) return { type: 'navigate', page: 'dashboard' };
            if (transcript.includes('كوموندات') || transcript.includes('طلبات')) return { type: 'navigate', page: 'commandes' };
            if (transcript.includes('منتوجات') || transcript.includes('سلع')) return { type: 'navigate', page: 'products' };
            if (transcript.includes('زبائن') || transcript.includes('كليان')) return { type: 'navigate', page: 'clients' };
            if (transcript.includes('كريدي') || transcript.includes('ديون') || transcript.includes('قائمة الكريدي')) { showVoiceResult('✅ أصلاً فالكريدي'); return { type: 'ignore' }; }
        }
        if ((transcript === 'حذف' || transcript === 'مسح' || transcript === 'طمس') && window.creditSelectedIndex >= 0 && window.creditSelectionMode) { return { type: 'delete_selected_credit' }; }
        if (transcript === 'حذف' || transcript === 'مسح' || transcript === 'طمس') { if (!window.creditSelectionMode) { window.creditSelectionMode = true; window.creditSelectedIndex = -1; window.creditPaymentStep = 'idle'; if (typeof window.renderCreditsTable === 'function') window.renderCreditsTable(); showVoiceResult('📋 وضع الاختيار'); return { type: 'ignore' }; } if (window.creditSelectedIndex < 0) { showVoiceResult('⚠️ ڭول الرقم'); return { type: 'ignore' }; } return { type: 'delete_selected_credit' }; }
        ...
        if (window.creditPaymentStep === 'payment' || window.creditPaymentStep === 'amount') { if (transcript.includes('صيفط') || transcript.includes('إيه') || transcript.includes('نعم') || transcript.includes('ok')) return { type: 'validate_credit_payment' }; return { type: 'unknown', text: transcript }; }
        var deleteMatch = transcript.match(/(?:حذف|مسح|طمس)\s+(?:سطر\s+)?(\d+|[^\s]+)/i); if (deleteMatch) { var delNum = parseInt(deleteMatch[1]); if (isNaN(delNum)) { for (var w in numberMap) { if (deleteMatch[1].toLowerCase() === w) { delNum = numberMap[w]; break; } } } if (!isNaN(delNum) && delNum > 0) return { type: 'delete_credit_line', lineNumber: delNum }; }
        if (transcript.includes('اختيار') || transcript.includes('حدد')) return { type: 'activate_credit_selection' };
        var lm2 = transcript.match(/(?:سطر|رقم)\s+([\u0600-\u06FF\u0030-\u0039]+)/i); if (lm2) { var ns2 = lm2[1], num2 = parseInt(ns2); if (isNaN(num2)) { for (var w in numberMap) { if (ns2.toLowerCase() === w) { num2 = numberMap[w]; break; } } } if (!isNaN(num2) && num2 > 0) return { type: 'select_credit_line', lineNumber: num2 }; }
        ...
        if (transcript.includes('خلاص') || transcript.includes('خلص') || transcript.includes('دفع')) return { type: 'mark_credit_paid' };
        var am = transcript.match(/مبلغ\s+(\d+[.,]?\d*)/i); if (am) { var a = parseFloat(am[1].replace(',', '.')); if (!isNaN(a) && a > 0) return { type: 'set_credit_amount', amount: a }; }
        if (transcript.includes('غلق') || transcript.includes('رجع')) return { type: 'close_credit_list' };
        return { type: 'unknown', text: transcript };
    }

    // Commandes de navigation globales en darija
    if (transcript.includes('كريدي') || transcript.includes('ديون') || transcript.includes('قائمة الكريدي')) return { type: 'navigate', page: 'credits' };
    if (transcript.includes('مبيعات') || transcript.includes('بيعات')) return { type: 'navigate', page: 'ventes' };
    if (transcript.includes('داشبورد') || transcript.includes('رئيسية') || transcript.includes('home')) return { type: 'navigate', page: 'dashboard' };
    if (transcript.includes('منتوجات') || transcript.includes('سلع') || transcript.includes('produits')) return { type: 'navigate', page: 'products' };
    if (transcript.includes('زبائن') || transcript.includes('كليان') || transcript.includes('clients')) return { type: 'navigate', page: 'clients' };
    if (transcript.includes('كوموندات') || transcript.includes('طلبات')) return { type: 'navigate', page: 'commandes' };
    if (transcript.includes('كاتيڭوري') || transcript.includes('أنواع')) return { type: 'navigate', page: 'categories' };
    if (transcript.includes('پوانت') || transcript.includes('كافي') || transcript.includes('كيسا') || transcript.includes('pos')) return { type: 'navigate', page: 'pos' };

    // Commandes générales (valider, annuler, passer, etc.) en darija
    ...
    if (transcript.includes('دوز') || transcript.includes('سويفانت') || transcript.includes('تالي')) return { type: 'next' };
    if (currentPage !== 'Crédits') { if (transcript.includes('صيفط') || transcript.includes('إيه') || transcript.includes('نعم') || transcript.includes('ok') || transcript.includes('valide')) return { type: 'validate' }; }
    if (transcript.includes('لا') || transcript.includes('تراجع') || transcript.includes('annuler')) return { type: 'cancel' };
    if (transcript.includes('فضي') || transcript.includes('مسح كولشي') || transcript.includes('vider')) return { type: 'clear' };
    if (transcript.includes('سالا') || transcript.includes('كمل') || transcript.includes('فيناليزي')) return { type: 'finalize' };

    // Recherche de produits / clients en darija
    if (window.posStep === 2) { if (window.posAllClients) { var fc3 = fastFindClient(transcript); if (fc3.length > 0) { var b3 = fc3[0]; for (var j = 0; j < fc3.length; j++) { if ((fc3[j].nom + ' ' + fc3[j].prenom).toLowerCase().indexOf(transcript) !== -1) { b3 = fc3[j]; break; } } return { type: 'client', client: b3 }; } } if (window.posProductsList) { var fp2 = null, bml2 = 0; for (var i2 = 0; i2 < window.posProductsList.length; i2++) { var p2 = window.posProductsList[i2]; var searchName = (p2.nom || '') + ' ' + (p2.nom_darija || ''); if (transcript.includes(searchName.toLowerCase()) && searchName.length > bml2) { fp2 = p2; bml2 = searchName.length; } } if (fp2) return { type: 'product', product: fp2 }; } }
    else { if (window.posProductsList) { var fp = null, bml = 0; for (var i = 0; i < window.posProductsList.length; i++) { var p = window.posProductsList[i]; var searchName = (p.nom || '') + ' ' + (p.nom_darija || ''); if (transcript.includes(searchName.toLowerCase()) && searchName.length > bml) { fp = p; bml = searchName.length; } } if (fp) return { type: 'product', product: fp }; } if (window.posAllClients) { var fc4 = fastFindClient(transcript); if (fc4.length > 0) { var b4 = fc4[0]; for (var j = 0; j < fc4.length; j++) { if ((fc4[j].nom + ' ' + fc4[j].prenom).toLowerCase().indexOf(transcript) !== -1) { b4 = fc4[j]; break; } } return { type: 'client', client: b4 }; } } }

    var am2 = transcript.match(/\d+[.,]?\d*/); if (am2) { var a = parseFloat(am2[0].replace(',', '.')); if (a > 0) return { type: 'amount', value: a }; }
    return { type: 'unknown', text: transcript };
}

// ==================== RECHERCHE CLIENTS EN DARIJA ====================
...
// ==================== GESTIONNAIRE DE COMMANDES (darija) ====================
function handleVoiceCommand(cmd) {
    ...
    switch (cmd.type) {
        ...
        case 'navigate':
            var p = cmd.page, pt = { 'credits': 'كريدي', 'ventes': 'مبيعات', 'dashboard': 'داشبورد', 'products': 'منتوجات', 'clients': 'زبائن', 'commandes': 'كوموندات', 'categories': 'أنواع', 'pos': 'كيسا' };
            if (cp === pt[p]) { showVoiceResult('✅ ' + pt[p]); hideVoiceFlowIndicator(); return; }
            if (typeof window.posCart !== 'undefined' && window.posCart.length > 0 && window.posStep === 1 && p !== 'pos') { if (!confirm('⚠️ الباني مازال فيه منتوجات. تفضيه؟')) { showVoiceResult('↩️ تراجع'); hideVoiceFlowIndicator(); return; } if (typeof window.posResetCart === 'function') window.posResetCart(); }
            ...
            break;
        case 'payment_mode':
            var m = cmd.mode; if ((m === 'credit' || m === 'partiel') && (!window.posCurrentClient || !window.posCurrentClient.id)) { alert('خاصك تختار زبون للكريدي/البارت'); showVoiceResult('⚠️ خاصك زبون'); hideVoiceFlowIndicator(); return; }
        ...
        case 'product':
            if (typeof window.posProductsList !== 'undefined' && window.posProductsList.length) {
                ...
                if (pr.stock !== undefined && pr.stock <= 0) { showVoiceResult('⚠️ نفذ من المخزون: ' + (pr.nom_darija || pr.nom)); hideVoiceFlowIndicator(); return; }
                if (typeof window.posAddToCartOrOpenOptions === 'function') window.posAddToCartOrOpenOptions(pr.id);
                showVoiceResult('✅ ' + (pr.nom_darija || pr.nom) + ' تزاد');
            ...
            if (voiceMode === 'quantity' && lastAddedProductId) {
                ...
                if (typeof window.posProductsList !== 'undefined') {
                    ...
                    if (it) {
                        ...
                        if (p2 && p2.stock !== undefined && qty > p2.stock) { showVoiceResult('⚠️ المخزون الأقصى: ' + p2.stock); hideVoiceFlowIndicator(); return; }
                        it.quantite = qty; lastAddedProductId = null;
                        setVoiceMode('search', '🎤 البحث الصوتي نشيط', null);
                        showVoiceResult('✅ الكمية: ' + qty + ' - ڭول منتوج آخر ولا "دوز"');
        ...
        case 'client':
            ...
            setVoiceMode('payment', '🎤 ڭول طريقة الخلاص ولا المبلغ', null); showVoiceResult('👤 ' + window.posCurrentClient.name);
        ...
        case 'clear': if (typeof window.posResetCart === 'function') { window.posResetCart(); showVoiceResult('🗑️ الباني تفضى'); if (typeof window.renderPOS === 'function') window.renderPOS(); hideVoiceFlowIndicator(); showVoiceFlowIndicator('product'); } break;
        case 'cancel': if (voiceMode !== 'search') { setVoiceMode('search', '🎤 البحث الصوتي نشيط', null); showVoiceResult('↩️ رجوع للبحث'); if (typeof window.renderPOS === 'function') window.renderPOS(); hideVoiceFlowIndicator(); showVoiceFlowIndicator('product'); } break;
...
// ==================== TOGGLE MICRO (darija) ====================
function posToggleVoiceSearch() { var s = checkVoiceSupport(); if (!s.supported) { alert('⚠️ ' + s.reason); return; } if (!navigator.onLine) { alert('⚠️ خاصك تكون متصل بالإنترنت'); return; } var mb = document.getElementById('posMicBtn'); if (isRecording) { posStopVoiceSearch(); return; } requestMicrophonePermission().then(function(p) { if (!p) { alert('❌ الميكروفون مرفوض'); return; } posStartVoiceRecording(); }); }
...
function posStartVoiceRecording() {
    ...
    var SR = window.SpeechRecognition || window.webkitSpeechRecognition; if (!SR) { alert('❌ التعرف الصوتي غير متوفر'); return; }
    voiceRecognition = new SR(); voiceRecognition.lang = 'ar'; // Arabe standard (base darija)
    voiceRecognition.continuous = true; voiceRecognition.interimResults = true; voiceRecognition.maxAlternatives = 1;
    ...
    voiceRecognition.onerror = function(e) { if (e.error === 'aborted' || e.error === 'no-speech') return; if (e.error === 'network') showVoiceResult('❌ مشكل فالشبكة'); posStopVoiceSearch(); };
    try { voiceRecognition.start(); isRecording = true; showVoiceModeIndicator(); showVoiceResult('🎤 سمع...'); showVoiceFlowIndicator('product'); } catch (e) { isRecording = false; if (mb) { mb.classList.remove('recording'); mb.innerHTML = '<i class="fas fa-microphone"></i>'; mb.style.background = '#dcfce7'; mb.style.borderColor = '#16a34a'; mb.style.boxShadow = 'none'; mb.style.transform = 'scale(1)'; mb.style.border = '3px solid #16a34a'; } }
}

function posStopVoiceSearch() { if (voiceRecognition) { try { voiceRecognition.abort(); } catch (e) {} voiceRecognition = null; } isRecording = false; var mb = document.getElementById('posMicBtn'), si = document.getElementById('posSearchInput'); if (mb) { mb.classList.remove('recording'); mb.innerHTML = '<i class="fas fa-microphone"></i>'; mb.style.background = '#dcfce7'; mb.style.borderColor = '#16a34a'; mb.style.boxShadow = 'none'; mb.style.transform = 'scale(1)'; mb.style.border = '3px solid #16a34a'; } if (si) { si.placeholder = '🔍 بحث...'; si.style.background = '#fff'; si.style.borderColor = '#e2e8f0'; si.style.boxShadow = 'none'; si.style.border = '2px solid #e2e8f0'; } var se = document.getElementById('voiceStyle'); if (se) se.remove(); var ind = document.getElementById('voiceModeIndicator'); if (ind) ind.remove(); hideVoiceFlowIndicator(); showVoiceResult('🎤 الميكرو متوقف'); }
...
window.onProductAdded = function(pid) { lastAddedProductId = pid; setVoiceMode('quantity', '🔢 الكمية', pid); showVoiceModeIndicator(); hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('quantity'); }, 200); };

console.log('🎤 Mixmax Minimarket - Module vocal Darija v9.0');
