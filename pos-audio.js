// ==================== POS-AUDIO.JS v8.1.3 AMÉLIORÉ - MIXMAX MINIMARKET ====================
// Commandes vocales enrichies : remise, impression, aide, mots-clés étendus
...
var paymentKeywords = {
    'espece': ['espèces', 'espece', 'argent', 'cash', 'comptant', 'liquide', 'espèce', 'en espèces'],
    'credit': ['crédit', 'credit', 'à crédit', 'acredit', 'dette', 'avance', 'crédit', 'à crédit'],
    'partiel': ['partiel', 'partielle', 'acompte', 'moitié', 'partial', 'part', 'partiel', 'paiement partiel']
};
...
// (toutes les fonctions de crédit restent identiques, je les omet pour la concision)
// ... (copie exacte de ta version originale pour ces fonctions)

// ==================== NOUVELLES FONCTIONS ====================
function applyDiscountVoice(amount) {
    if (typeof window.posUpdateDiscountMAD === 'function') {
        window.posUpdateDiscountMAD(amount);
        showVoiceResult('💰 Remise ' + amount.toFixed(2) + ' MAD');
    } else {
        showVoiceResult('⚠️ Remise impossible');
    }
}

function printLastFacture() {
    if (window.lastFactureId && typeof window.printFacture === 'function') {
        window.printFacture(window.lastFactureId);
        showVoiceResult('🖨️ Impression facture');
    } else {
        showVoiceResult('⚠️ Aucune facture récente');
    }
}
...
function speakHelp() {
    if ('speechSynthesis' in window) {
        var msg = new SpeechSynthesisUtterance(
            "Dites un produit pour l'ajouter. " +
            "Dites la quantité après le bip. " +
            "Dites passe ou valide pour continuer. " +
            "Dites efface ou vider pour vider le panier. " +
            "Dites remise suivi du montant. " +
            "Dites imprimer pour la facture. " +
            "Dites aide pour répéter."
        );
        msg.lang = 'fr-FR';
        msg.rate = 1.0;
        window.speechSynthesis.speak(msg);
    }
    showVoiceResult('📢 Aide énoncée');
}

// ==================== PARSER VOCAL v8.1.3 (enrichi) ====================
function parseVoiceCommand(transcript) {
    ...
    // 1. Commandes globales prioritaires (nouvelles)
    if (transcript === 'aide' || transcript === 'quoi dire' || transcript === 'commandes') return { type: 'help' };
    if (transcript === 'imprimer' || transcript === 'facture') return { type: 'print_facture' };
    if (transcript.includes('remise') || transcript.includes('rabais') || transcript.includes('réduction')) {
        var amt = transcript.match(/\d+([.,]\d+)?/);
        if (amt) {
            var val = parseFloat(amt[0].replace(',', '.'));
            if (val > 0) return { type: 'discount', amount: val };
        }
    }

    // 2. Détection des nombres / modes de paiement (inchangé)
    ...
    if (voiceMode === 'payment' || window.posStep === 2) {
        var pm2 = detectPaymentMode(transcript);
        if (pm2) return { type: 'payment_mode', mode: pm2 };
    }

    // 3. Commandes spécifiques aux pages (identiques à ta version, juste avec les mots-clés étendus)
    ...
    // Mots-clés étendus pour les actions
    if (transcript.includes('passe') || transcript.includes('passer') || transcript.includes('suivant') || transcript.includes('continue')) return { type: 'next' };
    if (currentPage !== 'Crédits') {
        if (transcript.includes('valide') || transcript.includes('validé') || transcript.includes('valider') || transcript.includes('confirmer') || transcript.includes('ok') || transcript.includes("c'est bon")) return { type: 'validate' };
    }
    if (transcript.includes('annule') || transcript.includes('annuler') || transcript.includes('retour') || transcript.includes('revenir')) return { type: 'cancel' };
    if (transcript.includes('efface') || transcript.includes('vider') || transcript.includes('tout retirer') || transcript.includes('supprimer tout')) return { type: 'clear' };
    if (transcript.includes('termine') || transcript.includes('terminer') || transcript.includes('fin') || transcript.includes('finaliser')) return { type: 'finalize' };
...
// ==================== GESTIONNAIRE DE COMMANDES (étendu) ====================
function handleVoiceCommand(cmd) {
    console.log('🎤', cmd.type); var cp = document.getElementById('pageTitle')?.textContent || '';
    // Nouvelles commandes en premier
    if (cmd.type === 'help') { speakHelp(); hideVoiceFlowIndicator(); return; }
    if (cmd.type === 'print_facture') { printLastFacture(); hideVoiceFlowIndicator(); return; }
    if (cmd.type === 'discount') { applyDiscountVoice(cmd.amount); hideVoiceFlowIndicator(); return; }
...
// ==================== EXPORTS (inchangés + nouvelles fonctions) ====================
...
window.applyDiscountVoice = applyDiscountVoice;
window.printLastFacture = printLastFacture;
window.speakHelp = speakHelp;

console.log('🎤 Module vocal v8.1.3 amélioré (remise, impression, aide)');
