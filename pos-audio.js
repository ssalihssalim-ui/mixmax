// ==================== POS-AUDIO.JS v8.1.2 DARIJA - MIXMAX MINIMARKET ====================
// Version originale v8.1.2 FINAL adaptée en darija marocain
...
// Mots-clés darija + français (pour compatibilité)
var paymentKeywords = {
    'espece': ['كاش', 'فلوس', 'نقد', 'ليكاش', 'كيش', 'espèces', 'espece', 'argent', 'cash', 'comptant', 'liquide'],
    'credit': ['كريدي', 'دين', 'سلف', 'بكريدي', 'crédit', 'credit', 'dette', 'avance'],
    'partiel': ['جزء', 'شوية', 'تقسيط', 'مويتي', 'بارت', 'partiel', 'partielle', 'acompte', 'moitié']
};

// Nombres en darija (prononciation + écriture)
var numberMap = {
    ...
    // Garder les mots français pour les utilisateurs qui les utilisent
    'wahed': 1, 'ouais': 1,'wad': 1, 'un': 1, 'une': 1,
    'juge': 2, 'joue': 2, 'george': 2, 'souche': 2, 'deux': 2,
    'claud': 3, 'cl': 3, 'trois': 3, 'clé': 3, 'clea': 3,
    'quatre': 4, 'rabah': 4, 'rabat': 4, 'rabats': 4, 'cinq': 5,
    'hamza': 5, 'rama': 5,
...
// ========== INDEX DE RECHERCHE CLIENT (LOCAL, AVEC DARIJA) ==========
...
// ==================== INDICATEUR DE PHASE (ANIMÉ, TEXTES DARIJA) ====================
...
// ==================== GESTION CRÉDITS (messages darija) ====================
...
function parseVoiceCommand(transcript) {
    ...
    if (currentPage === 'Ventes') {
        if (transcript.includes('اليوم') || transcript.includes('نهار') || transcript.includes("aujourd'hui") || transcript.includes('ce jour')) { window.ventesPeriod = 'today'; window.currentPages.ventes = 1; if (typeof window.applyVentesFilters === 'function') window.applyVentesFilters(); showVoiceResult('📅 اليوم'); return { type: 'ignore' }; }
        if (transcript.includes('هد الاسبوع') || transcript.includes('7 أيام') || transcript.includes('سيمانة') || transcript.includes('cette semaine') || transcript.includes('semaine')) { window.ventesPeriod = '7'; window.currentPages.ventes = 1; if (typeof window.applyVentesFilters === 'function') window.applyVentesFilters(); showVoiceResult('📅 7 أيام'); return { type: 'ignore' }; }
        if (transcript.includes('هد الشهر') || transcript.includes('30 يوم') || transcript.includes('شهر') || transcript.includes('ce mois') || transcript.includes('mensuel') || transcript.includes('mois')) { window.ventesPeriod = '30'; window.currentPages.ventes = 1; if (typeof window.applyVentesFilters === 'function') window.applyVentesFilters(); showVoiceResult('📅 30 يوم'); return { type: 'ignore' }; }
        if (transcript.includes('كلشي') || transcript.includes('تاريخ') || transcript.includes('جميع') || transcript.includes('tout') || transcript.includes('historique') || transcript.includes('global') || transcript.includes('complet')) { window.ventesPeriod = 'all'; window.currentPages.ventes = 1; if (typeof window.applyVentesFilters === 'function') window.applyVentesFilters(); showVoiceResult('📅 الكل'); return { type: 'ignore' }; }
        if (transcript.includes('اختيار') || transcript.includes('حدد') || transcript.includes('sélectionner') || transcript.includes('sélectionne') || transcript.includes('select') || transcript.includes('choisir') || transcript.includes('cocher')) { window.venteSelectionMode = true; window.venteSelectedIndex = -1; if (typeof window.renderVentesTable === 'function') window.renderVentesTable(); showVoiceResult('📋 وضع الاختيار'); return { type: 'ignore' }; }
        if (window.venteSelectionMode) {
            var lm = transcript.match(/(?:سطر|رقم|ligne|numéro)\s+([a-z0-9\u0600-\u06FF]+)/i); if (lm) { var ns = lm[1], num = parseInt(ns); if (isNaN(num)) { for (var w in numberMap) { if (ns.toLowerCase() === w) { num = numberMap[w]; break; } } } if (!isNaN(num) && num > 0) { window.venteSelectedIndex = num - 1; if (typeof window.renderVentesTable === 'function') window.renderVentesTable(); showVoiceResult('✅ سطر ' + num); return { type: 'ignore' }; } }
        ...
        if (transcript.includes('تفاصيل') || transcript.includes('ديتاي') || transcript.includes('détail') || transcript.includes('detail')) { if (window.venteSelectedIndex >= 0) { var data = window.filteredVentes || window.allVentesData || []; var vente = data[window.venteSelectedIndex]; if (vente) { if (typeof window.editVente === 'function') { window.editVente(vente.id); } else { showVoiceResult('📋 تفاصيل'); } return { type: 'ignore' }; } } showVoiceResult('⚠️ اختار سطر أولاً'); return { type: 'ignore' }; }
        if (transcript.includes('غلق') || transcript.includes('رجع') || transcript.includes('fermer') || transcript.includes('retour')) { if (window.venteSelectionMode) { window.venteSelectionMode = false; window.venteSelectedIndex = -1; if (typeof window.renderVentesTable === 'function') window.renderVentesTable(); showVoiceResult('📋 تقفل'); return { type: 'ignore' }; } }
        if (transcript.includes('پوانت') || transcript.includes('كافي') || transcript.includes('كيسا') || transcript.includes('pos') || transcript.includes('point de vente') || transcript.includes('caisse')) { return { type: 'navigate', page: 'pos' }; }
        if (transcript.includes('كريدي') || transcript.includes('ديون') || transcript.includes('crédits') || transcript.includes('impayés')) { return { type: 'navigate', page: 'credits' }; }
        if (transcript.includes('داشبورد') || transcript.includes('رئيسية') || transcript.includes('home') || transcript.includes('dashboard') || transcript.includes('accueil')) { return { type: 'navigate', page: 'dashboard' }; }
        if (transcript.includes('كوموندات') || transcript.includes('طلبات') || transcript.includes('commandes')) { return { type: 'navigate', page: 'commandes' }; }
        if (transcript.includes('منتوجات') || transcript.includes('سلع') || transcript.includes('produits') || transcript.includes('catalogue')) { return { type: 'navigate', page: 'products' }; }
        if (transcript.includes('زبائن') || transcript.includes('كليان') || transcript.includes('clients') || transcript.includes('clientèle')) { return { type: 'navigate', page: 'clients' }; }
        if (transcript.includes('مبيعات') || transcript.includes('بيعات') || transcript.includes('ventes') || transcript.includes('vente')) { showVoiceResult('✅ أصلاً فالمبيعات'); return { type: 'ignore' }; }
    ...
    if (currentPage === 'Crédits') {
        if (creditDeletePending) { if (transcript.includes('صيفط') || transcript.includes('إيه') || transcript.includes('نعم') || transcript.includes('ok') || transcript.includes('valide') || transcript.includes('validé') || transcript.includes('confirmer') || transcript.includes('oui')) { return { type: 'confirm_delete_credit' }; } if (transcript.includes('لا') || transcript.includes('تراجع') || transcript.includes('annule') || transcript.includes('annuler') || transcript.includes('non')) { return { type: 'cancel_delete_credit' }; } showVoiceResult('⚠️ صيفط ولا لا؟'); return { type: 'ignore' }; }
        if (window.creditPaymentStep !== 'payment' && window.creditPaymentStep !== 'amount') {
            if (transcript.includes('پوانت') || transcript.includes('كافي') || transcript.includes('كيسا') || transcript.includes('pos') || transcript.includes('point de vente') || transcript.includes('caisse')) return { type: 'navigate', page: 'pos' };
            if (transcript.includes('مبيعات') || transcript.includes('بيعات') || transcript.includes('ventes') || transcript.includes('vente')) return { type: 'navigate', page: 'ventes' };
            if (transcript.includes('داشبورد') || transcript.includes('رئيسية') || transcript.includes('home') || transcript.includes('dashboard') || transcript.includes('accueil')) return { type: 'navigate', page: 'dashboard' };
            if (transcript.includes('كوموندات') || transcript.includes('طلبات') || transcript.includes('commandes')) return { type: 'navigate', page: 'commandes' };
            if (transcript.includes('منتوجات') || transcript.includes('سلع') || transcript.includes('produits') || transcript.includes('catalogue')) return { type: 'navigate', page: 'products' };
            if (transcript.includes('زبائن') || transcript.includes('كليان') || transcript.includes('clients') || transcript.includes('clientèle')) return { type: 'navigate', page: 'clients' };
            if (transcript.includes('كريدي') || transcript.includes('ديون') || transcript.includes('crédits') || transcript.includes('impayés')) { showVoiceResult('✅ أصلاً فالكريدي'); return { type: 'ignore' }; }
        }
        if ((transcript === 'حذف' || transcript === 'مسح' || transcript === 'طمس' || transcript === 'supprimer' || transcript === 'supprime' || transcript === 'effacer' || transcript === 'enlever' || transcript === 'retirer') && window.creditSelectedIndex >= 0 && window.creditSelectionMode) { return { type: 'delete_selected_credit' }; }
        if (transcript === 'حذف' || transcript === 'مسح' || transcript === 'طمس' || transcript === 'supprimer' || transcript === 'supprime' || transcript === 'effacer' || transcript === 'enlever' || transcript === 'retirer') { if (!window.creditSelectionMode) { window.creditSelectionMode = true; window.creditSelectedIndex = -1; window.creditPaymentStep = 'idle'; if (typeof window.renderCreditsTable === 'function') window.renderCreditsTable(); showVoiceResult('📋 وضع الاختيار'); return { type: 'ignore' }; } if (window.creditSelectedIndex < 0) { showVoiceResult('⚠️ ڭول الرقم'); return { type: 'ignore' }; } return { type: 'delete_selected_credit' }; }
        ...
        if (window.creditPaymentStep === 'payment' || window.creditPaymentStep === 'amount') { if (transcript.includes('صيفط') || transcript.includes('إيه') || transcript.includes('نعم') || transcript.includes('ok') || transcript.includes('valide') || transcript.includes('validé') || transcript.includes('confirmer') || transcript.includes('oui')) return { type: 'validate_credit_payment' }; return { type: 'unknown', text: transcript }; }
        var deleteMatch = transcript.match(/(?:حذف|مسح|طمس|supprimer|supprime|effacer|enlever|retirer)\s+(?:سطر\s+|ligne\s+)?(\d+|[a-z\u0600-\u06FF]+)/i); if (deleteMatch) { var delNum = parseInt(deleteMatch[1]); if (isNaN(delNum)) { for (var w in numberMap) { if (deleteMatch[1].toLowerCase() === w) { delNum = numberMap[w]; break; } } } if (!isNaN(delNum) && delNum > 0) return { type: 'delete_credit_line', lineNumber: delNum }; }
        if (transcript.includes('اختيار') || transcript.includes('حدد') || transcript.includes('sélectionner') || transcript.includes('sélectionne') || transcript.includes('select') || transcript.includes('choisir') || transcript.includes('cocher')) return { type: 'activate_credit_selection' };
        var lm2 = transcript.match(/(?:سطر|رقم|ligne|numéro)\s+([a-z0-9\u0600-\u06FF]+)/i); if (lm2) { var ns2 = lm2[1], num2 = parseInt(ns2); if (isNaN(num2)) { for (var w in numberMap) { if (ns2.toLowerCase() === w) { num2 = numberMap[w]; break; } } } if (!isNaN(num2) && num2 > 0) return { type: 'select_credit_line', lineNumber: num2 }; }
        ...
        if (transcript.includes('خلاص') || transcript.includes('خلص') || transcript.includes('دفع') || transcript.includes('marquer payé') || transcript.includes('payer') || transcript.includes('régler')) return { type: 'mark_credit_paid' };
        var am = transcript.match(/(?:مبلغ|montant)\s+(\d+[.,]?\d*)/i); if (am) { var a = parseFloat(am[1].replace(',', '.')); if (!isNaN(a) && a > 0) return { type: 'set_credit_amount', amount: a }; }
        if (transcript.includes('غلق') || transcript.includes('رجع') || transcript.includes('fermer') || transcript.includes('retour')) return { type: 'close_credit_list' };
        return { type: 'unknown', text: transcript };
    }
    ...
    if (transcript.includes('زبائن') || transcript.includes('كليان') || transcript.includes('clients') || transcript.includes('clientèle')) return { type: 'navigate', page: 'clients' };
    if (transcript.includes('كوموندات') || transcript.includes('طلبات') || transcript.includes('commandes')) return { type: 'navigate', page: 'commandes' };
    if (transcript.includes('كاتيڭوري') || transcript.includes('أنواع') || transcript.includes('catégories')) return { type: 'navigate', page: 'categories' };
    if (transcript.includes('پوانت') || transcript.includes('كافي') || transcript.includes('كيسا') || transcript.includes('pos') || transcript.includes('point de vente') || transcript.includes('caisse')) return { type: 'navigate', page: 'pos' };
    ...
    if (transcript.includes('دوز') || transcript.includes('سويفانت') || transcript.includes('تالي') || transcript.includes('passe') || transcript.includes('passer') || transcript.includes('suivant')) return { type: 'next' };
    if (currentPage !== 'Crédits') { if (transcript.includes('صيفط') || transcript.includes('إيه') || transcript.includes('نعم') || transcript.includes('ok') || transcript.includes('valide') || transcript.includes('validé') || transcript.includes('confirmer')) return { type: 'validate' }; }
    if (transcript.includes('لا') || transcript.includes('تراجع') || transcript.includes('annule') || transcript.includes('annuler')) return { type: 'cancel' };
    if (transcript.includes('فضي') || transcript.includes('مسح كولشي') || transcript.includes('efface') || transcript.includes('vider')) return { type: 'clear' };
    if (transcript.includes('سالا') || transcript.includes('كمل') || transcript.includes('فيناليزي') || transcript.includes('termine') || transcript.includes('terminer') || transcript.includes('fin')) return { type: 'finalize' };
...
// ==================== GESTIONNAIRE DE COMMANDES (compatible darija+fr) ====================
...
// ==================== TOGGLE MICRO (LANGUE ARABE, MESSAGES DARIJA) ====================
...
console.log('🎤 Mixmax Minimarket - Module vocal Darija v8.1.2 chargé');
