// ==================== POS-AUDIO.JS v8.3 FINAL - RECONNAISSANCE VOCALE OPTIMISÉE ====================
// Mixmax Minimarket - 300ms recherche produit / 120ms quantité / 6ms redémarrage / 150ms inter-phase
// ✅ Phase produit = UNIQUEMENT recherche produit
// ✅ Phase client = UNIQUEMENT recherche client
...
function showVoiceFlowIndicator(phase) {
    ...
    setTimeout(function() {
        ...
        voiceFlowIndicator.style.cssText = 'position:fixed;top:12px;left:50%;transform:translateX(-50%);background:' + c.bg + ';border:3px solid ' + c.border + ';border-radius:30px;padding:12px 28px;font-size:1rem;font-weight:700;z-index:9998;box-shadow:0 4px 25px rgba(0,0,0,0.2);text-align:center;color:#1e293b;opacity:0;transition:opacity 0.12s;white-space:nowrap;';
        ...
        window._flowIndicatorTimeout = setTimeout(function() {
            if (voiceFlowIndicator) { voiceFlowIndicator.style.opacity = '0'; setTimeout(function() { if (voiceFlowIndicator && voiceFlowIndicator.parentNode) voiceFlowIndicator.remove(); voiceFlowIndicator = null; }, 150); }
        }, 3000);
    }, 120);
}
...
function hideVoiceFlowIndicator() {
    if (voiceFlowIndicator) {
        voiceFlowIndicator.style.opacity = '0';
        var old = voiceFlowIndicator;
        setTimeout(function() { if (old && old.parentNode) old.remove(); }, 150);
        voiceFlowIndicator = null;
    }
    voiceFlowPhase = 'idle';
}
...
// ==================== PARSER VOCAL v8.3 (PHASE PRODUIT/CLIENT PRIORITAIRES) ====================
function parseVoiceCommand(transcript) {
    ...
    // ===================================================================
    // PHASE PRODUIT (étape 1) : chercher UNIQUEMENT dans les produits
    // ===================================================================
    if (window.posStep === 1 && voiceMode === 'search') {
        // ✅ Navigation rapide (mots-clés courts)
        if (transcript === 'passe' || transcript === 'passer' || transcript === 'suivant') return { type: 'next' };
        if (transcript === 'valide' || transcript === 'validé' || transcript === 'valider' || transcript === 'confirmer' || transcript === 'ok') return { type: 'validate' };
        if (transcript === 'annule' || transcript === 'annuler') return { type: 'cancel' };
        if (transcript === 'efface' || transcript === 'vider') return { type: 'clear' };
        if (transcript === 'termine' || transcript === 'terminer' || transcript === 'fin') return { type: 'finalize' };
    ...
    // ===================================================================
    // PHASE CLIENT (étape 2) : chercher UNIQUEMENT dans les clients
    // ===================================================================
    if (window.posStep === 2) {
        // ✅ Mode paiement : UNIQUEMENT mots-clés
        if (voiceMode === 'payment') {
        ...
        if (window.posAllClients && voiceMode !== 'payment') {
...
// ==================== GESTIONNAIRE DE COMMANDES (150ms inter-phase) ====================
function handleVoiceCommand(cmd) {
    ...
    switch (cmd.type) {
        ...
        case 'payment_mode':
            ...
            hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('payment_amount'); }, 150);
            break;
        case 'product':
            if (typeof window.posProductsList !== 'undefined' && window.posProductsList.length) {
                ...
                hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('quantity'); }, 150);
            }
            break;
        case 'number':
            if (voiceMode === 'quantity' && lastAddedProductId) {
                ...
                if (typeof window.posProductsList !== 'undefined') {
                    ...
                    if (it) {
                        ...
                        hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('product'); }, 150);
                    }
                }
                ...
                if (typeof window.posAmountGiven !== 'undefined') {
                    ...
                    hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('confirm'); }, 150);
                }
            }
            break;
        case 'client':
            ...
            hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('payment_mode'); }, 150);
            break;
        case 'amount': if (window.posStep === 2) { if (typeof window.posAmountGiven !== 'undefined') { window.posAmountGiven = cmd.value; var ce2 = document.getElementById('posChangeDisplay'); if (ce2) { var st2 = typeof window.posCalculateTotal === 'function' ? window.posCalculateTotal() : 0, t2 = st2 - (window.posDiscountMAD || 0), c2 = window.posAmountGiven - t2; ce2.innerHTML = c2 >= 0 ? '<div class="pos-change-positive"><span>Rendu</span><span>' + c2.toFixed(2) + ' MAD</span></div>' : '<div class="pos-change-negative"><span>Manquant</span><span>' + Math.abs(c2).toFixed(2) + ' MAD</span></div>'; } var ai2 = document.getElementById('posAmountGiven'); if (ai2) ai2.value = window.posAmountGiven; showVoiceResult('💰 ' + window.posAmountGiven.toFixed(2) + ' MAD'); hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('confirm'); }, 150); } } break;
        case 'next': if (isModalOpen()) break; if (voiceMode === 'quantity') { setVoiceMode('search', '🎤 Recherche vocale active', null); if (typeof window.updateCartOnly === 'function') window.updateCartOnly(); showVoiceModeIndicator(); hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('product'); }, 150); } else if (window.posStep === 2) { if (typeof window.posFinalizeSale === 'function') window.posFinalizeSale(); } else if (window.posCart && window.posCart.length > 0 && window.posStep === 1) { if (typeof window.posGoToStep2 === 'function') window.posGoToStep2(); hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('client'); }, 150); } break;
        case 'validate': if (isModalOpen()) break; if (voiceMode === 'quantity') { setVoiceMode('search', '🎤 Recherche vocale active', null); if (typeof window.updateCartOnly === 'function') window.updateCartOnly(); showVoiceModeIndicator(); hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('product'); }, 150); } else if (window.posStep === 2) { if (typeof window.posFinalizeSale === 'function') { window.posFinalizeSale(); hideVoiceFlowIndicator(); } } else if (window.posStep === 1 && window.posCart && window.posCart.length > 0) { if (typeof window.posGoToStep2 === 'function') window.posGoToStep2(); hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('client'); }, 150); } break;
...
// ==================== TOGGLE MICRO (6ms redémarrage / 300ms recherche / 120ms quantité) ====================
...
function posStartVoiceRecording() {
    ...
    voiceRecognition.onresult = function(e) {
        ...
        // ✅ 300ms recherche produit/client, 120ms quantité/montant
        var debounceDelay = (voiceMode === 'quantity' || voiceMode === 'payment' || window.posStep === 2) ? 120 : 300;
    ...
    // ✅ Redémarrage micro 6ms
    voiceRecognition.onend = function() { if (isRecording) { setTimeout(function() { try { voiceRecognition.start(); } catch (e) { posStopVoiceSearch(); } }, 6); } };
...
window.onProductAdded = function(pid) { lastAddedProductId = pid; setVoiceMode('quantity', '🔢 Qté', pid); showVoiceModeIndicator(); hideVoiceFlowIndicator(); setTimeout(function() { showVoiceFlowIndicator('quantity'); }, 150); };

console.log('🎤 Mixmax Minimarket - Module vocal v8.3 FINAL (phases priorisées + 300ms/120ms/6ms/150ms)');
