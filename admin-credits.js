...
function loadCreditsPage(c) {
    ...
    if (!sortOrders.credits.createdAt) { sortOrders.credits.createdAt = 'desc'; }
    
    // Charger les clients pour la recherche vocale
        ...
        db.collection('clients').limit(500).get().then(function(snap) {
            ...
            snap.forEach(function(d) {
                ...
                window.posAllClients.push({
                    id: d.id, nom: data.nom || '', prenom: data.prenom || '',
                    telephone: data.telephone || '', description: data.description || ''
                });
            });
...
function updateCreditPaymentZone() {
    ...
    if (window.creditSelectedIndex >= 0 && window.creditPaymentStep !== 'idle') {
        ...
        if (credit) {
            ...
            if (input) { input.value = window.creditPaymentAmount > 0 ? window.creditPaymentAmount : ''; input.focus(); input.select(); }
            return;
        }
    }
