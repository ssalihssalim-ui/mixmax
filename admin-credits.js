...
// Gestion des crédits – Version simple (aucune sélection multiple, aucun bouton "Tout cocher")
...
async function loadCreditsPage(c) {
    ...
    // Chargement des clients pour la recherche dropdown (ne bloque pas l'affichage)
    ...
    // Construction de l'interface (sans bouton de sélection multiple)
    ...
    // Lancement immédiat du chargement des crédits
    loadCredits();
}
...
async function loadCredits() {
    ...
    // 1. Afficher d'abord les données en cache
    ...
    // 2. Si en ligne, synchroniser avec Firestore et mettre à jour le cache
    if (navigator.onLine) {
        try {
...
console.log('🛒 Mixmax Minimarket - Admin Credits chargé (simple, hors ligne)');
