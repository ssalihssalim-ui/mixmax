...
async function loadPosPage(c) {
    ...
    c.innerHTML = '<div style="text-align:center;padding:60px;">' +
        ...
        '<p style="margin-top:15px;color:#64748b;">Chargement du POS...</p></div>';
    ...
    setTimeout(async function() {
        try {
            ...
            // ✅ Vérifier qu'on est toujours sur la page POS avant de render
            if (isOnPOSPage()) {
                renderPOS();
            }
        } catch (e) {
...
// ✅ Nouvelle fonction : vérifier si on est sur la page POS
function isOnPOSPage() {
    var pageTitle = document.getElementById('pageTitle')?.textContent || '';
    return pageTitle === 'POS' || pageTitle === 'Dashboard';
}
...
function filterProductGrid() {
    if (!isOnPOSPage()) return; // ✅ Protection
...
// ==================== RENDU DU POS ====================
function renderPOS() {
    // ✅ Protection : ne pas render si on n'est plus sur la page POS
    if (!isOnPOSPage()) {
        console.log('⏭️ Rendu POS ignoré - page actuelle:', document.getElementById('pageTitle')?.textContent);
        return;
    }

    // ✅ Anti-re-rendu rapide (évite les doubles rendus en moins de 100ms)
    var now = Date.now();
    if (now - posLastRenderTime < 100 && posCart.length > 0) {
        console.log('⏭️ Rendu POS ignoré - trop rapide');
        return;
    }
...
function buildFullPOS(c) {
    if (posProductsList.length === 0 && posCategoriesList.length === 0) {
        c.innerHTML = '<div style="text-align:center;padding:40px;">' +
            ...
            '<p style="margin-top:12px;">Chargement du POS...</p></div>';
        return;
    }
    ...
    // Barre de recherche + micro + boutons
    ...
    // Grille produits
    h += '<div class="pos-products-grid" id="posProductGrid"></div></div>';
    ...
    if (posStep === 1) {
        ...
        if (posCart.length === 0) {
            h += '<div class="pos-cart-empty"><i class="fas fa-shopping-basket"></i><p>Panier vide</p></div>';
        } else {
            for (var k = 0; k < posCart.length; k++) {
                var it = posCart[k];
                var opts = '';
...
async function posFinalizeSale() {
    if (isFinalizing) { console.log('⏳ Finalisation déjà en cours'); return; }
    ...
    isFinalizing = true;
    var finalizeBtn = document.querySelector('.pos-finalize-btn');
    if (finalizeBtn) { finalizeBtn.disabled = true; finalizeBtn.textContent = '⏳ Finalisation...'; }
    ...
    try {
        ...
        if (posPaymentMethod === 'credit') {
            ...
            posAmountGiven = parseFloat(document.getElementById('posAmountGiven').value) || 0;
            remaining = t - posAmountGiven; paid = false; statutPaiement = 'partiel';
            change = Math.max(0, posAmountGiven - t);
        } else {
            posAmountGiven = parseFloat(document.getElementById('posAmountGiven').value) || 0;
            change = posAmountGiven - t; paid = true; statutPaiement = 'payé';
        }
        ...
        var itemsDetail = posCart.map(function(it) {
            ...
            return {
                ...
                sauces: [], interdits: it.interdits || [], epice: it.epice || 'Normal', sel: it.sel || 'Normal'
            };
        });
        ...
        var sd = {
            ...
            table: posCurrentTable || null, vendeur: vendeur,
            paymentMethod: posPaymentMethod, statutPaiement: statutPaiement,
            amountGiven: posAmountGiven, change: change, paid: paid, remainingAmount: remaining,
        ...
        if (window.posCommandeId) {
            batch.update(db.collection('commandes').doc(window.posCommandeId), { statut: 'payé', paidAt: firebase.firestore.FieldValue.serverTimestamp(), factureNum: fn });
            delete window.posCommandeId;
        }
        if (window.posVenteId) {
            batch.update(db.collection('ventes').doc(window.posVenteId), { paid: true, statutPaiement: 'payé', remainingAmount: 0, paidAt: firebase.firestore.FieldValue.serverTimestamp() });
            delete window.posVenteId;
        }
        ...
        posResetCart();
        if (isOnPOSPage()) renderPOS();
        if (navigator.onLine) { setTimeout(function() { CacheDB.sync().catch(function(e) {}); }, 500); }

    } catch (e) { alert('Erreur: ' + e.message); }
    finally {
        isFinalizing = false;
        if (finalizeBtn) { finalizeBtn.disabled = false; finalizeBtn.innerHTML = '<i class="fas fa-check-circle"></i> Finaliser'; }
    }
}
...
// ⚡ RACCOURCIS CLAVIER (protégé contre les doublons)
if (!window._posKeydownListenerAdded) {
    window._posKeydownListenerAdded = true;
...
window.onProductAdded = window.onProductAdded || function(productId) {
    console.log('Produit ajouté:', productId);
};

console.log('⚡ Mixmax Minimarket - POS (métier) chargé (v2 corrigée)');
