...
// ========== INDEX PRODUIT (ADMIN) – AVEC DESCRIPTION ==========
function fastFindProductAdmin(query) {
    ...
    // Construire un index temporaire (nom + description)
    var adminIndex = {};
    window.allProductsData.forEach(function(p) {
        ...
        nom.split(/[\s,;.]+/).forEach(function(w) {
            ...
            if (!adminIndex[w].includes(p)) adminIndex[w].push(p);
        });
    ...
    if (filtered.length === 0) {
        // Aucun ne contient les deux mots exacts, on retourne le premier candidat (le plus probable)
        return [candidates[0]];
    }

    if (filtered.length === 1) return filtered;

    // Plusieurs candidats, on cherche un match exact dans le nom
    var exact = filtered.find(function(p) {
        var nom = (p.nom || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
        return nom === searchTerm;
    });
    ...
    // Sinon, on prend le plus court (le plus probable)
    filtered.sort(function(a, b) { return (a.nom||'').length - (b.nom||'').length; });
    return [filtered[0]];
}
